...
HEADERS = {'User-Agent': 'LeadHunterApp/1.0 (personal lead generation tool)'}
    ...
    for tag_key, tag_val in _osm_tags(category)[:2]:
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f'[overpass] {city} HTTP {resp.status_code}')
                time.sleep(3)
                continue
            ...
            for el in elements:
                ...
                phone   = t.get('phone') or t.get('contact:phone') or t.get('telephone')
                ...
                leads.append({
                    ...
                    'city':        city,
                    'country':     country,
                    'priority':    'LOW' if has_website else 'HOT',
                    'profile_link': None,
...
def search_region(category, region, max_cities=6):
    ...
    all_leads.sort(key=lambda x: (0 if x['priority'] == 'HOT' else 1, x.get('name') or ''))
    return all_leads
